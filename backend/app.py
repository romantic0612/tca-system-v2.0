...
def get_frontend_dir():
    ...
    Vue is now the default entry for the V2.0 product shell. The proven static
    pages remain available under /legacy/* so the demo keeps feature parity
    while the Vue rewrite continues page by page.
    """
    frontend_mode = os.getenv('TCA_FRONTEND_MODE', 'vue').strip().lower()
...
def serve_frontend_file(frontend_dir, filename):
    ...
    if filename.startswith('legacy/'):
        legacy_name = filename[len('legacy/'):]
        legacy_path = os.path.join(STATIC_DIR, legacy_name)
        if os.path.exists(legacy_path) and os.path.isfile(legacy_path):
            return send_from_directory(STATIC_DIR, legacy_name)
        abort(404)
